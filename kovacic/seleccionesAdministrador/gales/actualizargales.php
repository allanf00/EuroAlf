<?php 
	...
	if ( null==$id ) {
		header("Location: administradorgales.php");
	}
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE gales  set dorsal = ?, posicion = ?, nombre =?, apellidos =?, edad = ?, partidosjugados = ?, goles=?, amarillas=?, rojas=? WHERE id = ?";
			$q = $pdo->prepare($sql);
		...
		$pdo = Database::connect();
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT * FROM gales where id = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($id));
...
<body>
    <div class="container">
    			...
    			<div class="span10 offset1">
	    			...
	    			<form class="form-horizontal" action="actualizargales.php?id=<?php echo $id?>" method="post">
					  ...
					  <div class="form-actions">
						  ...
						  <a class="btn" href="administradorgales.php">Volver</a>
						</div>
					</form>
				</div>
