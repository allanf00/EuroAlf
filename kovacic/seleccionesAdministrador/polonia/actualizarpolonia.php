<?php 
	...
	if ( null==$id ) {
		header("Location: administradorpolonia.php");
	}
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE polonia  set dorsal = ?, posicion = ?, nombre =?, apellidos =?, edad = ?, partidosjugados = ?, goles=?, amarillas=?, rojas=? WHERE id = ?";
			$q = $pdo->prepare($sql);
		...
		$pdo = Database::connect();
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT * FROM polonia where id = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($id));
...
<body>
    <div class="container">
    			...
    			<div class="span10 offset1">
	    			...
	    			<form class="form-horizontal" action="actualizarpolonia.php?id=<?php echo $id?>" method="post">
					  ...
					  <div class="form-actions">
						  ...
						  <a class="btn" href="administradorpolonia.php">Volver</a>
						</div>
					</form>
				</div>
