<?php 
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE grupoc  set posicion = ?, nombre = ?, partidosjugados =?, ganados =?, empatados = ?, perdidos = ?, golesfavor=?, golescontra=?, puntos=? WHERE id = ?";
			$q = $pdo->prepare($sql);
...
<body>
    <div class="container">
    			...
    			<div class="span10 offset1">
	    			...
	    			<form class="form-horizontal" action="actualizargrupoc.php?id=<?php echo $id?>" method="post">
					  ...
					  <div class="form-actions">
						  ...
						  <a class="btn" href="administradorgrupoc.php">Volver</a>
						</div>
					</form>
				</div>
